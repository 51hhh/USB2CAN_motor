...
- `omni_chassis_control_node` 只负责把 `/cmd_vel` 转换成电机命令，不再发布轮速积分里程计。
...
### 3.1.1 `/cmd_vel` 和 `/odom` 的区别

| Topic | 消息类型 | 是位置吗 | 含义 |
|---|---|---|---|
| `/cmd_vel` | `geometry_msgs/Twist` | 不是 | 给底盘的**速度命令**，例如前进速度、横移速度、旋转角速度 |
| `/odom` | `nav_msgs/Odometry` | 是 | STM32 码盘定位桥发布的最终里程计，包含位置 `pose` 和速度 `twist` |
...
3. **启动底盘运动学控制**：`omni_chassis_control_node`。
...
| 看纯码盘 `/odom` | `positioning_bridge_node` | `/odom` 只来自 STM32 码盘定位桥 |
...
含义：节点启动后不会自动发送 `SET_LOCAL_ORIGIN(0,0,0)`，避免 `/odom` 在下游闭环运行期间发生一次性跳变。

如需启动时自动归零，把 `auto_reset_origin_on_start` 改成 `true`。自动归零会让 `/odom` 从 MCU 当前累计值跳到接近 0，这是**正常的一次性跳变**。
...
当前 `/odom` 只由 `positioning_bridge_node` 读取 STM32 码盘定位板数据后发布，不再叠加电机轮速积分预测。

| 数据 | Topic/来源 | 作用 |
|---|---|---|
| STM32 码盘定位 | 串口 ODOM_STATE | `/odom` 的位置和姿态来源 |
| 输出结果 | `/odom` | ROS 里程计与 `odom -> base_link` TF |
...
当前 `/odom` 是纯 STM32 码盘输出；如果仍然抖动，优先检查串口帧质量、时间同步和是否有多个 `/odom` 发布者。
